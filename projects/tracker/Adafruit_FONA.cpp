...
boolean Adafruit_FONA::begin(Stream &port, int FONA_PWRKEY)
{
  mySerial = &port;
  ...
  DEBUG_PRINT(F("\t---> "));
  DEBUG_PRINTLN("AT+GMR"); // This definitely should have the module name, but ATI may not
...
boolean Adafruit_FONA::enablePSM(bool onoff, char *TAU_val, char *activeTime_val)
{ // AT+CPSMS command
...
boolean Adafruit_FONA::setNetLED(bool onoff, uint8_t mode, uint16_t timer_on, uint16_t timer_off)
{
  if (onoff)
  {
    if (!sendCheckReply(F("AT+CNETLIGHT=1"), ok_reply))
      return false;
...
boolean Adafruit_FONA::setAudio(uint8_t a)
{
  ...
  if ((_type == SIM5320A || _type == SIM5320E) && (a != 1 && a != 3 && a != 4))
    return false;
  // For SIM7500, 1 is headset, 3 is speaker phone
  else if ((_type == SIM7500 || _type == SIM7600) && (a != 1 && a != 3))
    return false;
...
boolean Adafruit_FONA::getGPS(float *lat, float *lon, float *speed_kph, uint16_t *heading, char *sats)
{
...
boolean Adafruit_FONA::enableGPRS(boolean onoff)
{
  ...
  else if (_type == SIM7070)
  {
    ...
    if (!openWirelessConnection(onoff))
      return false;
...
boolean Adafruit_FONA::openWirelessConnection(bool onoff)
{
  ...
  { // Disconnect wireless
    if (_type == SIM7070)
    {
      if (!sendCheckReply(F("AT+CNACT=0,0"), ok_reply))
        return false;
    }
    else
    {
      if (!sendCheckReply(F("AT+CNACT=0"), ok_reply))
        return false;
    }
    ...
    if (_type == SIM7070 && strstr(replybuffer, ",DEACTIVE") == NULL)
      return false; // +APP PDP: <pdpidx>,DEACTIVE
    else if (_type == SIM7000 && strstr(replybuffer, "PDP: DEACTIVE") == NULL)
  ...
  else
  {
    if (_type == SIM7070)
    {
      if (!sendCheckReply(F("AT+CNACT=0,1"), ok_reply))
        return false;
    }
    else
    {
      // if (! sendCheckReplyQuoted(F("AT+CNACT=1,"), apn, ok_reply)) return false;
      if (!sendCheckReply(F("AT+CNACT=1"), ok_reply))
        return false;
    }
    ...
    if (_type == SIM7070 && strstr(replybuffer, ",ACTIVE") == NULL)
      return false; // +APP PDP: <pdpidx>,ACTIVE
    else if (_type == SIM7000 && strstr(replybuffer, "PDP: ACTIVE") == NULL)
...
boolean Adafruit_FONA::postData(const char *request_type, const char *URL, const char *body, const char *token, uint32_t bodylen)
{
  ...
  else if (String(request_type) == "POST" && bodylen > 0)
...
boolean Adafruit_FONA::postData(const char *server, uint16_t port, const char *connType, const char *URL, const char *body)
{
  ...
  if (_type == SIM7500 || _type == SIM7600)
  {
    ...
    if ((strstr(replybuffer, "+CHTTPSOPSE: 0") == NULL) || (strstr(replybuffer, "+CHTTPSOPSE:0") == NULL))
      return false;
  }
  else
  {
...
boolean Adafruit_FONA_LTE::HTTP_connect(const char *server)
{
  ...
  sendCheckReply(F("AT+SHCONF=\"BODYLEN\",63"), ok_reply, 10000); // Max 1024 for SIM7070G (firmwork 1529B01SIM7000G supports only upto 63)
  ...
  if (!sendCheckReply(F("AT+SHCHEAD"), ok_reply, 10000))
    return false;
...
const char *Adafruit_FONA::FTP_GET(const char *fileName, const char *filePath, uint16_t numBytes)
{
  ...
  else
  {
    ...
    getReply(auxStr, 10000);
    if (strstr(replybuffer, "+FTPEXTGET: 2,") == NULL)
      return err;
  }
...
boolean Adafruit_FONA_LTE::MQTT_setParameter(const char *paramTag, const char *paramValue, uint16_t port)
{
  ...
  if (strcmp(paramTag, "CLIENTID") == 0 || strcmp(paramTag, "URL") == 0 || strcmp(paramTag, "TOPIC") == 0 || strcmp(paramTag, "MESSAGE") == 0)
  {
    ...
    if (!sendCheckReply(cmdStr, ok_reply))
      return false;
  }
  else
  {
...
{
  if (yesno)
    return sendCheckReply(F("AT+SMCONN"), ok_reply, 5000);
  else
    return sendCheckReply(F("AT+SMDISC"), ok_reply);
}
...
boolean Adafruit_FONA_LTE::MQTT_publish(const char *topic, const char *message, uint16_t contentLength, byte QoS, byte retain)
{
  ...
  if (strstr(replybuffer, ">") == NULL)
    return false; // Wait for "> " to send message
  if (!sendCheckReply(message, ok_reply, 5000))
...
boolean Adafruit_FONA::TCPconnect(char *server, uint16_t port)
{
  if (SSL_FONA)
  {
    ...
    if (!sendCheckReply(F("AT+CMEE=2"), ok_reply))
      return false;
    ...
    if (!sendCheckReply(F("AT+CACID=1"), ok_reply))
      return false;
    CID_CA_FONA = 1;

    //Configure SSL Parameters of a Context Identifier
    if (!sendCheckReply(F("AT+CSSLCFG=\"sslversion\",1,3"), ok_reply))
      return false;
    if (!sendCheckReply(F("AT+CSSLCFG=\"protocol\",1,1"), ok_reply))
      return false;
    ...
    if (!sendCheckReply(F("AT+CFSINIT"), ok_reply))
      return false;
    ...
    if (!sendCheckReply(F("AT+CFSTERM"), ok_reply))
      return false;
    if (!sendCheckReply(F("AT+CFSINIT"), ok_reply))
      return false;
    ...
    if (!sendCheckReply(F("AT+CSSLCFG=\"convert\",2,\"ca.crt\""), ok_reply))
      return false;

    //Set SSL certificate and timeout parameters
    if (!sendCheckReply(F("AT+CASSLCFG=1,\"cacert\",\"ca.crt\""), ok_reply))
      return false;
    if (!sendCheckReply(F("AT+CASSLCFG=1,\"ssl\",1"), ok_reply))
      return false;
    if (!sendCheckReply(F("AT+CASSLCFG=1,\"crindex\",1"), ok_reply))
      return false;
    if (!sendCheckReply(F("AT+CASSLCFG=1,\"protocol\",0"), ok_reply))
      return false;
