...
#--------------------------------------------------------------------
def FSMbattery::setup(Sim7000 *sim_device):
    time(&now)
    stateChange = now
...
def FSMbattery::run():
    ...
    case READ_GPS:
        if moving:
            time(&now)
            if now - stateChange > (TGPS * MIN_TO_S_FACTOR): # No GPS data in the time window allowed
                gpsErrors += 1
    ...
    case SEND_DATA:
        time(&now)
        if now - stateChange > (Tsend * MIN_TO_S_FACTOR):                      # Data upload not achieved
